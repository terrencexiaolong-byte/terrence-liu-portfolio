...
function InterestsPage() {
  ...
  return <main className="interests-page">
    ...
    <section className="gallery-wrap" id="photography">
      <div className="gallery-intro"><div className="gallery-copy"><p className="eyebrow">PHOTOGRAPHY / SELECTED WORKS</p><h2>旅行与日常的光线记录</h2><p>以下照片墙收录的是我的其他摄影作品；它们与独立完成的纸本摄影书《Islands》分别呈现。</p></div></div>
      <article className="photobook-showcase"><div className="photobook-cover"><img src={sitePath('photobook/page-01.jpg')} alt="《Islands》摄影书封面" loading="eager" decoding="async"/><span>ISLANDS</span></div><div className="photobook-showcase-copy"><p className="eyebrow">INDEPENDENT PHOTOBOOK / 2024</p><h2>一册从版面<br/>到纸张的影像叙事。</h2><p>我使用 InDesign 完成《Islands》的设计与编排，并将它印刷成册。线上版本以双页阅读的方式，延续纸本翻阅的节奏。</p><a className="photobook-entry" href={sitePath('photobook.html')}><span><small>OPEN ISLANDS · 45 PAGES</small>进入完整摄影集</span><b>↗</b></a></div></article>
      <div className="gallery-grid-label"><p>OTHER PHOTOGRAPHIC WORKS</p><h2>独立摄影作品</h2></div>
      ...
      <div className="photobook-outro"><div className="photobook-mark"><div><strong>捡树枝的鸦</strong><small>凝万象于帧　栖一心在影</small></div><img src={sitePath('brand/logo-mark.png')} alt="捡树枝的鸦标志" /></div><p>INDEPENDENT PHOTOBOOK</p><h2>想翻阅我亲自<br/>设计的摄影书？</h2><a className="photobook-entry photobook-entry--outro" href={sitePath('photobook.html')}><span><small>ISLANDS · 45 PAGES · DOUBLE-PAGE READER</small>开始阅读摄影集</span><b>↗</b></a></div>
    </section>
    <FitnessHero />
...
function PhotoBookPage() {
  ...
  const goToSpread = (nextIndex) => {
    ...
    if (boundedIndex === spreadIndex) return
    setMotion(boundedIndex > spreadIndex ? 'next' : 'previous')
  ...
  useEffect(() => {
    const onKeyDown = (event) => {
      ...
      if (event.key === 'Home') goToSpread(0)
      if (event.key === 'End') goToSpread(totalSpreads - 1)
    }
  ...
  return <main className="photobook-page">
    ...
    <section className="photobook-stage" onTouchStart={(event) => { swipeStart.current = event.touches[0]?.clientX ?? null }} onTouchEnd={(event) => {
      ...
      if (Math.abs(distance) > 42) goToSpread(spreadIndex + (distance < 0 ? 1 : -1))
      ...
      <button className="photobook-turn photobook-turn--next" type="button" onClick={() => goToSpread(spreadIndex + 1)} disabled={spreadIndex === totalSpreads - 1} aria-label="下一跨页">→</button>
    </section>
    <footer className="photobook-footer">
      <p>双页阅读 · 使用 ← →、空格键或滑动翻页</p>
      <div className="photobook-progress" role="group" aria-label="影集阅读进度">
        <div className="photobook-progress-heading"><span>READING PROGRESS</span><b>···</b></div>
        <div className="photobook-progress-summary"><strong>{Math.round(((spreadIndex + 1) / totalSpreads) * 100)}%</strong><span>{String(spreadIndex + 1).padStart(2, '0')} / {String(totalSpreads).padStart(2, '0')}</span></div>
        <div className="photobook-progress-track" aria-label={`第 ${spreadIndex + 1} 跨页，共 ${totalSpreads} 跨页`}>
...
function App() {
  ...
  return <main>
    ...
    <section className="work section motion-reveal" id="work"><div className="frame"><div className="section-top" data-motion="SELECTED WORK"><p className="eyebrow">03 — SELECTED WORK</p><p className="section-note">研究 / 系统 / 模型</p></div><div className="work-intro"><h2>让数据<br/><span>成为视野。</span></h2><p>精选项目聚焦于市场预测、风险系统与金融建模。每一个项目都从具体问题出发，落向更好的决策。</p></div></div><div className="project-rail">{projects.map((p) => <article className={`project-card ${p.type}`} key={p.no}><div className="card-visual"><span className="card-no">{p.no}</span>{p.type === 'signal' ? <button className="result-frame result-frame--signal" type="button" onClick={() => setActiveResult({src:sitePath('projects/stock-model-results.jpg'), alt:'股票预测模型完整输出结果', label:'STOCK FORECAST / FULL OUTPUT'})}><img src={sitePath('projects/stock-model-results.jpg')} alt="股票预测模型训练与价格拟合结果" loading="lazy" fetchPriority="low" decoding="async"/><span className="result-label">点击查看完整结果 ↗</span></button> : p.type === 'risk' ? <div className="result-frame result-frame--risk"><img className="risk-exposure" src={sitePath('projects/factor-exposure.jpg')} alt="匿名化投资组合因子敞口分析结果" loading="lazy" fetchPriority="low" decoding="async"/><img className="risk-trends" src={sitePath('projects/factor-trends.jpg')} alt="" loading="lazy" fetchPriority="low" decoding="async"/><span className="risk-anonymized-label">ANONYMIZED PORTFOLIO</span><span className="risk-trends-label">ANONYMOUS FACTOR TREND</span><span className="result-label">FACTOR EXPOSURE / TREND</span></div> : <div className="financial-model" aria-label="匿名化三表联动财务预测示意"><div className="financial-model-head"><span>ANONYMISED FINANCIAL MODEL</span><b>FY24 — FY26</b></div><div className="financial-chart"><span className="chart-label">REVENUE / OPERATING CASH FLOW</span><i/><i/><i/><i/><i/><svg viewBox="0 0 260 78" aria-hidden="true"><polyline points="0,62 42,54 83,57 125,36 166,40 207,17 260,9"/><polyline className="cash-line" points="0,67 42,63 83,59 125,54 166,47 207,38 260,25"/></svg></div><div className="statement-grid">{[['INCOME STATEMENT','Revenue','Gross Margin','EBITDA'],['BALANCE SHEET','Working Capital','Capex','Net Debt'],['CASH FLOW','Operating CF','Investing CF','Free Cash Flow']].map(([title,...rows]) => <div className="statement" key={title}><strong>{title}</strong>{rows.map((row,index) => <span key={row}>{row}<b>{['12.4','48.2%','3.1'][index]}</b></span>)}</div>)}</div><span className="financial-disclaimer">ILLUSTRATIVE DATA · NOT CLIENT INFORMATION</span></div>}</div><div className="card-content"><p className="eyebrow">{p.en}</p><h3>{p.title}</h3><p>{p.text}</p>{p.specs && <dl className="project-specs">{p.specs.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>}<div className="tags">{p.tags.map(x => <span key={x}>{x}</span>)}</div><button aria-label={`查看${p.title}`}><ArrowUpRight size={20}/></button></div></article>)}</div></section>
